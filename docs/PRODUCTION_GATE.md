...
**Date:** 2026-08-12  
**Verdict:** **GO for pilot / investor demo** (with known limitations below)  
**Baseline:** stages 0–29 + frontend consolidation (20) + XSS (21)
...
| Area | Status |
|------|--------|
| Build & tests | ✅ API + Web production builds green; unit **182** / security **85** pass |
| Security foundation | ✅ Fail-fast secrets, headers, throttle, KYC/Media ACL tested |
...
> Live Stripe, production secrets, HTTPS reverse proxy и `SENTRY_DSN` нужно выставить в целевом окружении (чеклист ниже). Полный delete legacy HTML — после pilot confirmation.

---

## 1. Build & Tests

| Check | Status | Evidence (2026-08-12) |
|-------|--------|------------------------|
| `apps/api` `npm run build` | ✅ | `nest build` exit 0 |
| `apps/web` `npm run build` | ✅ | Next 16.3 production build, 51 routes |
| Unit tests (`apps/api` `npm test`) | ✅ | **39** suites, **182** tests passed |
| Security suite (`npm run test:security`) | ✅ | **15** suites, **85** tests passed |
...
| Security headers | ✅ | Helmet CSP/HSTS/frame/nosniff; Next + nginx headers (Stage 22); `security-headers.spec.ts` |
| Rate limiting | ✅ | Global + hard limits Redis store; `throttle.limits.spec.ts` |
| KYC access | ✅ | `kyc-acl.spec.ts` — cross-shop 403, admin OK |
...
| Dependency CVE scan | 🟡 | Recommend `npm audit` in CI before each release (not a code-path blocker) |
...
|-------|--------|----------|
| Stripe test/live mode | 🟡 | Provider abstraction + env; **set keys in deploy env**; test mode OK for pilot |
...
| Legacy HTML | 🟡 | Fallback only; not on nginx default path |
